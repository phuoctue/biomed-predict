...
import { Activity, BarChart3, Calendar, Eye, FileClock, Pill, RefreshCw } from "lucide-react";
...
export const UsageStatisticsPage = () => {
  ...
  // Hàm xử lý reset và tải lại dữ liệu
  const handleReset = () => {
    setDateFrom("");
    setDateTo("");
    refetch();
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-8 antialiased">
      <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Thống kê sử dụng</h1>
          <p className="mt-1 text-sm font-medium text-slate-600">
            Theo dõi lượt xem thuốc, trang truy cập nhiều và các hành động người dùng trong hệ thống.
          </p>
        </div>

        <div className="flex flex-col gap-3 rounded-2xl border border-slate-100 bg-white p-3 shadow-sm sm:flex-row sm:items-end">
          <label className="flex flex-col gap-1 text-xs font-bold uppercase tracking-wider text-slate-500">
            ...
            <div className="relative flex items-center">
              <input
                ...
                className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-800 outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-400"
              />
              <Calendar className="pointer-events-none absolute right-3 text-slate-400" size={16} />
            </div>
          </label>
          ...
          <button
            onClick={handleReset}
            className="inline-flex h-[38px] items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-2 text-sm font-bold text-white transition hover:bg-blue-700"
          >
            <RefreshCw size={16} />
            Tải lại
          </button>
          ...
          return (
            <div key={metric.label} className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-center justify-between">
                ...
                {loading && <span className="text-[10px] font-bold uppercase text-slate-400">Đang tải</span>}
              </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
        ...
        {loading ? (
          <div className="py-16 text-center text-sm font-medium text-slate-500">Đang tải hoạt động...</div>
        ) : logs.length === 0 ? (
          <div className="py-16 text-center text-sm font-medium text-slate-500">Chưa có dữ liệu hoạt động.</div>
        ) : (
            ...
            <tbody className="divide-y divide-slate-100">
              {logs.map((log) => (
                <tr key={log.id} className="transition hover:bg-slate-50">
                  <td className="p-4 text-sm font-semibold text-slate-800">{log.userName ?? "--"}</td>
                  <td className="p-4 text-sm font-semibold text-slate-700">{log.action ?? "--"}</td>
                  <td className="p-4 text-sm text-slate-600">{log.entityType ?? log.page ?? "--"}</td>
                  <td className="p-4 text-sm text-slate-600">
                    {log.createdAt ? new Date(log.createdAt).toLocaleString("vi-VN") : "--"}
                  </td>
                  <td className="p-4 text-sm text-slate-600">{log.ipAddress ?? "--"}</td>
                </tr>
              ))}
            </tbody>
          </table>
