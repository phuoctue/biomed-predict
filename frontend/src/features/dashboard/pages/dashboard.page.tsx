...
const RiskBadge = ({ value }: { value?: string }) => {
  ...
  return (
    <span
      className={`rounded-full border px-2.5 py-1 text-[10px] font-black uppercase ${className}`}
    >
      {normalized}
    </span>
  );
...
const MetricCard = ({
  ...
  const tone =
    metricTone[metric.key as keyof typeof metricTone] ??
...
export const DashboardPage = () => {
  ...
  // Hàm xử lý reset date và tải lại dữ liệu
  const handleReset = () => {
    setDateFrom("");
    setDateTo("");
    refetch();
      ...
      <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Theo dõi bệnh nhân, đơn thuốc, cảnh báo AI và trạng thái cấp phát
            trong hệ thống.
          </p>
        </div>
          ...
          <label className="flex flex-col gap-1 text-xs font-bold uppercase tracking-wider text-slate-400">
            ...
            <div className="relative flex items-center">
              <input
                ...
                className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-900 outline-none focus:border-blue-400"
              />
              <Calendar
                ...
                {evaluations.map((evaluation) => (
                  ...
                  >
                    ...
                    <td className="p-4 text-sm font-black text-slate-800">
                      {evaluation.suitabilityScore != null
